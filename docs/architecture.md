...
- 不直接访问串口或 Client 地址。
- 提供 PC 与手机两套响应式布局：视口 >768px 为常驻侧边栏桌面布局；≤768px 切换为手机布局（顶栏汉堡按钮 + 左侧抽屉导航 + 遮罩，内容单列堆叠，Agent 面板全屏覆盖，弹窗近全屏，触控目标 ≥44px）。由 `index.html` / `styles.css` / `app.js` 实现，无独立构建管线。
...
- Client 到 Server 默认使用 WS；启用 TLS 后使用 WSS。Client Token 认证由 Server 侧 `TTLAB_CLIENT_AUTH_ENABLED` 与 Client 配置 `client.json` 的 `authEnabled` 共同控制，启用时使用每 Client 独立的可撤销凭据。
